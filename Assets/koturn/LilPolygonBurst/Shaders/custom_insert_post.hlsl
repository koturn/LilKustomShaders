...
void geomCustom(triangle v2g input[3], inout TriangleStream<v2f> outStream, uint primitiveID : SV_PrimitiveID)
{
    ...
    // #if defined(LIL_ONEPASS_FUR)
        ...
        const float cycleTime = fmodglsl(getTime(), oneCycleTime);
    ...
    if (cycleTime >= polyBurstStartTime) {
        ...
        for (uint i = 0; i < 3; i++) {
            input[i].positionWS = rotate3D(input[i].positionWS, centerPos, polyNormal, getTime() * _PolygonRotSpeed * primitiveRand.y) + moveWS;
        }
    }
